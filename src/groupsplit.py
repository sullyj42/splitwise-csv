...
def split(total, num_people):
    ...
    ), "InternalError:" + " something doesnt add up here: %d * %d + %d != %d" % (
        base,
        num_people,
        extra,
        total,
...
class Splitwise:
    ...
    def get_client(self):
        ...
        uri, headers, body = client.sign(
            "https://secure.splitwise.com/api/v3.0/get_access_token", http_method="POST"
        )
    ...
    def get_id(self):
        if not hasattr(self, "my_id"):
            resp = self.api_call(
                "https://secure.splitwise.com/api/v3.0/get_current_user", "GET"
            )
    ...
    def delete_expense(self, expense_id):
        return self.api_call(
            "https://secure.splitwise.com/api/v3.0/delete_expense/%s" % expense_id,
            "POST",
        )
    ...
    def get_expenses(self, after_date="", limit=0, allow_deleted=True):
        ...
        resp = self.api_call(
            "https://secure.splitwise.com/api/v3.0/get_expenses?%s" % (paramsStr), "GET"
        )
        if not allow_deleted:
            resp["expenses"] = [
                exp for exp in resp["expenses"] if exp["deleted_at"] is None
            ]
...
class CsvSettings:
    def __init__(self, rows):
        ...
        self.has_title_row = (
            raw_input("Does first row have titles? [Y/n]").lower() != "n"
        )
        self.newest_transaction = ""
        while True:
            try:
                self.local_currency = raw_input(
...
class SplitGenerator:
    ...
    def make_transactions(self):
        ...
        **change csvDateFormat to the format in your csv if necessary**
        ...
        for r in self.rows:
            if (
                not self.options.try_all
                and do_hash(str(r)) == self.csv.newest_transaction
            ):
                break
                ...
                self.transactions.append(
                    {
                        "date": datetime.strftime(
                            datetime.strptime(r[self.csv.date_col], csvDateFormat),
                            "%Y-%m-%dT%H:%M:%SZ",
                        ),
                        "amount": -1
                        * Money(r[self.csv.amount_col], self.csv.local_currency),
    ...
    def get_group(self, name):
        ...
        for group in groups:
            if group["name"].lower() == name.lower():
                gid = group["id"]
                members = [
                    m["id"] for m in group["members"] if m["id"] != self.api.get_id()
                ]
                num_found += 1
    ...
    def ask_for_splits(self):
        ...
        for t in self.transactions:
            if (
                self.options.yes
                or raw_input(
                    "%d: %s at %s $%s. Split? [y/N]"
                    % (i, t["date"], t["desc"], t["amount"])
                ).lower()
                == "y"
            ):
        ...
        assert (
            self.options.yes or raw_input("Confirm submission? [y/N]").lower() == "y"
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.members)):
            ...
            params["users__%s__owed_share" % (i + 1)] = (
                (base + one_cent).amount if extra.amount > 0 else base.amount
            )
            extra -= one_cent
        paramsStr = urllib.urlencode(params)
